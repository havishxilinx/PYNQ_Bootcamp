...
| `notebooks/PYNQ_302-Referee_Match_Client.ipynb` | The real competition client — detection (all five approaches, switchable via a dropdown) + wire protocol + Genesis, in one notebook. Useful for comparing approaches on a single board. |
| `notebooks/PYNQ_302-<approach>-<Red\|Blue>.ipynb` | Same client, but with one detection approach fixed (no dropdown) and `TEAM_NAME` pre-set to `'red'`/`'blue'` — 10 files total (`yolo_full_frame`, `yolo_grid_crops`, `aruco_border`, `aruco_per_card`, `aruco_border_grid_crops`, x 2 colors). `aruco_border_grid_crops` combines `aruco_border`'s corner auto-calibration with `yolo_grid_crops`'s per-cell cropping instead of full-frame detection. Shows a fixed stage tracker (Join -> Riddle -> Free Hint -> Play/Wait sub-stages) instead of free-form status text, for a simpler widget GUI during a live demo. The color names are unrelated to Genesis's own `team_red`/`team_blue` arm assignment, which the referee decides dynamically per match. |
